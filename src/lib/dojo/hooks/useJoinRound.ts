...
export const useJoinRound = (): UseJoinRoundReturn => {
  ...
  const joinRound = useCallback(async (roundId: bigint) => {
    ...
    try {
      // Get round data from store using Dojo's entity ID format
      const roundEntityId = getEntityIdFromKeys([roundId]);
      const roundIdHex = `0x${roundId.toString(16)}`;
      console.log('[useJoinRound] Looking for round in store:', { 
        roundEntityId, 
        roundIdHex,
        roundId: roundId.toString()
      });
      
      // Try Dojo entity ID first (matches optimistic updates and network data)
      let round = store.entities[roundEntityId]?.models?.lyricsflip?.Rounds as Rounds | undefined;
      
      // Fallback to hex format for backwards compatibility
      if (!round) {
        round = store.entities[roundIdHex]?.models?.lyricsflip?.Rounds as Rounds | undefined;
        console.log('[useJoinRound] Fallback hex lookup:', { found: !!round });
      }
